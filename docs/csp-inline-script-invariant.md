...
1. Any `<script>` **without `src`** whose `type` is missing, empty, or a
   JavaScript MIME type (`module`, `text/javascript`, `application/javascript`,
   plus the legacy aliases browsers also execute).
2. Any inline event handler attribute (`onclick=`, `onerror=`, …) anywhere in the
   markup — including on a `<script>` tag, which is where the one we ship lives.
3. Any `.wasm` file in the export. `WebAssembly.compile/instantiate` needs
...
It does **not** fail on: `style=` attribute counts, `http://localhost` references,
or the `script-src` line itself. Those are printed every run for drift
visibility, because they are one decision away from becoming the next problem.

## Today's numbers (on `main`)

```
html files scanned        154
<script src=…>            2179   external — allowed by script-src 'self'
inline <script>           154    [application/json×152, module×1, <missing>×1]
  data blocks             152
  executable              2
inline event handlers     1
style= attributes         2494   informational
.wasm files               0
...
CSP today**. The exception exists so the gate can land and stay green while it
enforces the invariant everywhere else; the fix is deleting the file, not
...
The gate's own tests (`tests/csp/inline-scripts.test.mjs`, 23 cases) cover the
fail cases, the must-not-false-positive cases (comment-inert markup, markup
inside a quoted attribute value, a bare `<` in text), the exception/stale-exception
