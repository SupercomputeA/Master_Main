...
export async function onRequest({ request, env }) {
  ...
  if (!env?.CACHE) {
    return json({ error: "Server misconfigured: CACHE binding missing" }, 503, request);
  }
  ...
  if (path.startsWith("/snapchain/")) {
    ...
    try {
      ...
      return json(data, res.status, request, rlHeaders);
    } catch (e) {
      return json({ error: "Snapchain API error", detail: e.message }, 502, request, rlHeaders);
    }
  }
  ...
  if (path === "/info") {
    try {
      ...
      return json({ error: "Failed to fetch Neynar info", detail: e.message }, 502, request, rlHeaders);
    }
  }
  ...
  if (path === "/casts") {
    ...
    if (!fid) return json({ error: "fid query param required" }, 400, request, rlHeaders);
    ...
    try {
      ...
      return json(data, res.status, request, res.ok ? { ...rlHeaders, "Cache-Control": PUBLIC_CACHE } : rlHeaders);
    } catch (e) {
      return json({ error: "Failed to fetch Neynar feed", detail: e.message }, 502, request, rlHeaders);
    }
  }
  ...
  if (path === "/user") {
    ...
    if (!address) return json({ error: "address query param required" }, 400, request, rlHeaders);
    ...
    try {
      ...
      return json({ error: "Failed to fetch user", detail: e.message }, 502, request, rlHeaders);
    }
  }
