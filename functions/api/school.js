// functions/api/school.js — School curriculum API
...
const SCHOOL_MODULES = [
  {
    ...
    lessons: [
      ...
      { id: "WS-01-03", title: "Phishing & Scams", description: "How to identify and avoid common Web3 phishing attacks and social engineering.", duration: "25 min", topics: "Phishing identification, Social engineering, Fake dApps, Approval scams" },
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: "AS-01-03", title: "Token-Gated Agent Access", description: "Restricting agent actions and data access based on token holdings.", duration: "25 min", topics: "Token gating, $QUANTA staking, Access tiers, Balance verification" },
...
export async function onRequest({ request, env }) {
  ...
  // GET /api/school — list all modules (summary)
  // GET /api/school?id=DF-01 — single module with lessons
  // GET /api/school?id=DF-01&lesson=DF-01-01 — single lesson
  // GET /api/school?tracks=free — filter by access level

  const moduleId = url.searchParams.get("id")
  const lessonId = url.searchParams.get("lesson")
  const trackFilter = url.searchParams.get("tracks")

  let modules = SCHOOL_MODULES
  if (trackFilter === "free") modules = modules.filter(m => m.access === "free")
  if (trackFilter === "member") modules = modules.filter(m => m.access === "member")

  // Try D1 first for progress tracking (if user is authenticated)
  // For now, serve from static data — D1 integration for progress tracking later

  if (moduleId) {
    const mod = modules.find(m => m.moduleId === moduleId)
    if (!mod) return j({ error: "module not found" }, 404)

    if (lessonId) {
      const lesson = mod.lessons.find(l => l.id === lessonId)
      if (!lesson) return j({ error: "lesson not found" }, 404)
      return j({ module: { moduleId: mod.moduleId, title: mod.title }, lesson })
    }
  ...
  return j({
    ...
    stats: {
      ...
      totalLessons,
      freeModules: modules.filter(m => m.access === "free").length,
      memberModules: modules.filter(m => m.access === "member").length,
      credentials: modules.filter(m => m.credential).length,
    },
  })
}
