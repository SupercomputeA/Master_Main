...
export async function onRequest({ request, env }) {
  ...
  if (path === "/queue/update" && request.method === "POST") {
    ...
    if (payload.status !== undefined) {
      if (typeof payload.status !== "string" || !QUEUE_STATUS_SET.has(payload.status)) {
        return json({ error: "invalid status", allowed: QUEUE_STATUSES }, 400, request, env)
      }
  ...
  if (path === "/publish" && request.method === "POST") {
    ...
    return json({ id: item.id, status, results, forced: payload.force === true }, 200, request, env)
  }
