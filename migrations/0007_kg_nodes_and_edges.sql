...
-- the relational shape (kg_nodes + kg_edges) backed by D1 so the four
-- graphs (school, police, defi, articles) can be queried, joined, and
-- searched against the same store that holds articles/projects/sessions.
--
-- Domain count target (per CLAUDE.md P0.3): > 9 entities per domain.
-- Seed script (scripts/seed-kg-data.mjs) loads 18+ nodes / 20+ edges per
...
CREATE TABLE IF NOT EXISTS kg_nodes (
  id TEXT PRIMARY KEY,
  graph TEXT NOT NULL,           -- 'school' | 'police' | 'defi' | 'articles'
  label TEXT NOT NULL,
