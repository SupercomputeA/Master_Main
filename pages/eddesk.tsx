...
import { modules, type Lesson } from "../lib/school"
...
export default function EdDesk() {
  const [tab, setTab] = useState<"home" | "builder">("home")
  ...
  return (
    <Layout title="SUPERCOMPUTE · EdDesk">
      <section className="hero" id="eddesk">
        <div className="hero-kicker"><div className="status-dot"></div><span className="label">// eddesk</span></div>
        <h1 className="display-xl hero-title">EDUCATION<br /><em>DESK</em></h1>
        <p className="hero-sub">Curated modules, credentials, and course management for the Supercompute School. Build and publish educational content.</p>
      </section>

      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid var(--border)", padding: "0 24px" }}>
        {(["home", "builder"] as const).map((t) => (
        ...
        <>
          <section className="section">
            <div className="section-header">
              ...
              <div><h2 className="display-md">Education Desk</h2></div>
            </div>
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", padding: "24px" }}>
              <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.7, marginBottom: 16 }}>
                The Education Desk manages all curriculum, credentials, and learning paths for the Supercompute School.
                Browse published modules, track lesson progress, and manage student access.
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 1, background: "var(--border)", border: "1px solid var(--border)" }}>
                <div style={{ background: "var(--bg)", padding: "20px" }}>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--accent)", marginBottom: 4 }}>// modules</div>
                  <div className="display-md" style={{ fontSize: 32 }}>{modules.length}</div>
                  <div style={{ fontSize: 11, color: "var(--muted)" }}>Published</div>
                </div>
                <div style={{ background: "var(--bg)", padding: "20px" }}>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--accent)", marginBottom: 4 }}>// lessons</div>
                  <div className="display-md" style={{ fontSize: 32 }}>{modules.reduce((sum, m) => sum + m.lessons.length, 0)}</div>
                  <div style={{ fontSize: 11, color: "var(--muted)" }}>Total</div>
                </div>
                <div style={{ background: "var(--bg)", padding: "20px" }}>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--accent)", marginBottom: 4 }}>// access</div>
                  <div className="display-md" style={{ fontSize: 32, color: "var(--accent)" }}>{modules.filter(m => m.access === "free").length} Free</div>
                  <div style={{ fontSize: 11, color: "var(--muted)" }}>{modules.filter(m => m.access === "member").length} Member</div>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="section">
            <div className="section-header">
              ...
              <div><h2 className="display-md">Published Modules</h2></div>
            </div>
        ...
        <>
          <section className="section">
            <div className="section-header">
              ...
              <div><h2 className="display-md">Module Schema</h2></div>
            </div>
  ...
  topics: string[]
          ...
          <section className="section">
            <div className="section-header">
              ...
              <div><h2 className="display-md">Create a Module</h2></div>
            </div>
