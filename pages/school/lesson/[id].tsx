...
import type { SchoolModule, Lesson } from "../../../lib/school"

function findLesson(id: string): { module: SchoolModule; lesson: Lesson; index: number } | undefined {
  for (const m of modules) {
    const idx = m.lessons.findIndex(l => l.id === id)
    if (idx !== -1) return { module: m, lesson: m.lessons[idx], index: idx }
  }
}
...
export default function LessonPage() {
  ...
  const found = typeof id === "string" ? findLesson(id) : undefined

  if (!found) {
    return (
      <Layout title="SUPERCOMPUTE · Lesson Not Found">
        <section className="hero">
          <div className="hero-kicker"><div className="status-dot"></div><span className="label">// error</span></div>
          <h1 className="display-xl hero-title">LESSON<br /><em>NOT FOUND</em></h1>
          <p className="hero-sub">No lesson with ID "{id}" exists.</p>
  ...
  const { module: mod, lesson: l, index } = found
  const prev = index > 0 ? mod.lessons[index - 1] : null
  const next = index < mod.lessons.length - 1 ? mod.lessons[index + 1] : null

  return (
    <Layout title={`SUPERCOMPUTE · ${l.title}`}>
      <section className="hero" style={{ paddingBottom: 48 }}>
        <div className="hero-kicker">
          <div className="status-dot"></div>
          <span className="label" style={{ color: mod.color }}>// {mod.id} · lesson {String(index + 1).padStart(2, "0")}</span>
        </div>
        <div style={{ display: "flex", alignItems: "baseline", gap: 16, marginBottom: 8 }}>
          <Link href={`/school/${mod.id}`} style={{
        ...
        <p className="hero-sub" style={{ maxWidth: 600, marginBottom: 24 }}>
          {l.description}
        </p>
      ...
      <section className="section">
        <div className="section-header">
          ...
          <div>
            <h2 className="display-md">Key Topics</h2>
          </div>
        </div>
      ...
      <section className="section">
        <div className="section-header">
          ...
          <div>
            <h2 className="display-md">Lesson Content</h2>
          </div>
        </div>
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", padding: "32px" }}>
          <div style={{ fontSize: 13, color: "var(--fg)", lineHeight: 1.8, maxWidth: 720 }}>
            <p style={{ marginBottom: 20 }}>
              This lesson covers <strong style={{ color: "var(--accent)" }}>{l.title.toLowerCase()}</strong> —
              part of the <strong style={{ color: mod.color }}>{mod.title}</strong> module.
            </p>
            <p style={{ marginBottom: 20 }}>{l.description}</p>
      ...
      <section className="section" style={{ borderBottom: "none" }}>
        <div className="section-header">
          ...
          <div>
            <h2 className="display-md">Continue Learning</h2>
          </div>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
          ...
          <Link href={`/school/${mod.id}`} style={{
            ...
            <div style={{ fontSize: 11, color: "var(--fg)" }}>{mod.id} {mod.title}</div>
          </Link>
          {next ? (
...
export type { SchoolModule, Lesson }