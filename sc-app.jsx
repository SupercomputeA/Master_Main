...
const ARTICLES = [
  ...
  { id: '4', title: 'Cloudflare Workers + D1: Building at the Edge', excerpt: 'Our decision to build the QUANTA API on Cloudflare Workers was driven by speed, cost, and zero cold-start latency at the global edge.', category: 'signal', author: 'Quanta S', views: 411, published_at: Date.now() / 1000 - 259200, icon: '🔧' },
  { id: '5', title: 'Web3 Community Building: The QUANTA Playbook', excerpt: 'After 3 years building in Web3, we have developed a systematic approach to community growth that combines human insight with AI-native automation.', category: 'intelligence', author: 'Quanta S', views: 3105, published_at: Date.now() / 1000 - 432000, icon: '🌐' },
];
  ...
  return (
    <div className="auth-overlay">
      <div className="auth-card">
        ...
        <p style={{ fontSize: '1.3rem', fontWeight: 800, color: '#E2E8F0', marginBottom: '.3rem' }}>Welcome back</p>
        <p style={{ fontSize: '.8rem', color: '#64748B', marginBottom: '1.5rem', lineHeight: 1.5 }}>
        ...
        <button className="auth-btn" onClick={() => { if (window.doLogin) window.doLogin(); else window.showToast('Wallet auth required'); }}>{tab === 'signin' ? 'Sign In' : 'Create Account'}</button>
        <div className="auth-div">or</div>
        <button className="siwe-btn" onClick={() => { if (window.doLogin) window.doLogin(); else window.showToast('Wallet auth required'); }}>
          <span style={{ fontSize: 14 }}>◎</span> Sign In with Ethereum
        </button>
        {isDev && (
        <div style={{ marginTop: '1rem', fontSize: 11, color: '#64748B' }}>
          <button onClick={() => doDemo('user')} style={{ border: 'none', background: 'none', color: 'var(--gold)', cursor: 'pointer', fontSize: 11, fontWeight: 600, textDecoration: 'underline', fontFamily: 'inherit' }}>Demo: User Login</button>
          &nbsp;·&nbsp;
          <button onClick={() => doDemo('admin')} style={{ border: 'none', background: 'none', color: 'var(--gold)', cursor: 'pointer', fontSize: 11, fontWeight: 600, textDecoration: 'underline', fontFamily: 'inherit' }}>Demo: Admin Login</button>
        </div>
        )}
      </div>
    </div>
...
function DashboardPage({ auth }) {
  ...
  return (
    <>
      <TopBar title="Dashboard" subtitle="Mario Jefferson · quantas.molt.eth" />
      <div className="pg-body">
        {/* School banner */}
        {pct < 100 && (
          <div className="card mb-6" style={{ background: 'var(--navy)', border: 'none', color: '#fff' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: 48, height: 48, borderRadius: 0, background: 'rgba(255,255,255,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>🎓</div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontWeight: 700, fontSize: 13 }}>Complete Web3 School to Access TradeDesk</span>
              ...
              <button onClick={() => window.navigate('web3school')} style={{ padding: '.6rem 1.1rem', background: 'rgba(255,255,255,.2)', border: 'none', borderRadius: 0, color: '#fff', fontWeight: 600, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'inherit' }}>Continue →</button>
            </div>
          </div>
        )}
        ...
        <div className="grid4 mb-6">
          {[
            { ico: '📰', label: 'NewsDesk', desc: 'Content · Quanta S', page: 'newsdesk', c: 'var(--gold)' },
            ...
            { ico: '🎓', label: 'Web3 School', desc: 'Learn fundamentals', page: 'web3school', c: 'var(--cyan)' },
          ].map(q => (
            <div key={q.label} className="stat-card card-hover" style={{ cursor: 'pointer' }} onClick={() => window.navigate(q.page)}>
              <div style={{ width: 44, height: 44, borderRadius: 0, background: `${q.c}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: '.85rem' }}>{q.ico}</div>
        ...
        <div className="grid2">
          ...
          <Card>
            ...
            <div style={{ marginTop: '.75rem', padding: '.75rem', background: 'rgba(200,168,75,.04)', borderRadius: 0, border: '1px solid rgba(200,168,75,.12)' }}>
...
function NewsDeskPage() {
  ...
  return (
    <>
      ...
      <div className="pg-body">
        {/* Quanta status */}
        <div className="card mb-6" style={{ background: 'linear-gradient(135deg,rgba(200,168,75,.05),rgba(255,184,0,.05))', borderColor: 'rgba(200,168,75,.2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: 48, height: 48, borderRadius: 0, background: 'var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>✨</div>
              <div>
        ...
        <div className="grid4 mb-6">
          {[
            ...
            <div key={s.label} className="stat-card" style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
              <div style={{ width: 34, height: 34, borderRadius: 0, background: 'rgba(200,168,75,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>{s.ico}</div>
              <div>
            ...
            <div style={{ display: 'flex', gap: '.4rem', flexWrap: 'wrap' }}>
              {cats.map(c => (
                <button key={c.id} onClick={() => setFilter(c.id)}
                  style={{
                    padding: '.3rem .75rem', borderRadius: 0, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: '1px solid', fontFamily: 'inherit',
                    borderColor: filter === c.id ? 'var(--gold)' : 'var(--border)',
          ...
          <div className="modal-card" style={{ maxWidth: 680 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.8 }}>
                This article was autonomously generated and published by Quanta S. The full content covers advanced concepts in {selectedArticle.category} with actionable insights for the QUANTA community and beyond.
              </p>
              <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'rgba(200,168,75,.04)', borderRadius: 10, border: '1px solid rgba(200,168,75,.12)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: 'var(--muted)' }}>
          ...
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: 'var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>✨</div>
                <div>
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              <div style={{ padding: '1rem', background: 'rgba(200,168,75,.04)', border: '1px solid rgba(200,168,75,.15)', borderRadius: 10, marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', gap: '.75rem' }}>
                <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: '.75rem', border: '1px solid var(--border)', borderRadius: 0, fontWeight: 600, cursor: 'pointer', background: 'transparent', color: 'var(--text)', fontFamily: 'inherit' }}>Cancel</button>
                <button onClick={() => { setShowModal(false); window.showToast('Article queued for Quanta S review'); }} style={{ flex: 1, padding: '.75rem', background: 'var(--gold)', color: '#fff', border: 'none', borderRadius: 0, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Publish →</button>
              </div>
            </div>
          </div>
        </div>
...
function TradeDeskPage() {
  ...
  return (
    <>
      ...
      <div className="pg-body">
        {/* KNIGHT status */}
        <div className="card mb-6" style={{ background: 'linear-gradient(135deg,rgba(0,212,255,.05),rgba(200,168,75,.05))', borderColor: 'rgba(0,212,255,.2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: 48, height: 48, borderRadius: 0, background: 'var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>⚡</div>
              <div>
        ...
        <div className="grid2 mb-6">
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--gold)' }}>${p.long_exposure.toLocaleString()}</div>
            </div>
          </div>
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(200,168,75,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>📉</div>
            <div>
              <div className="stat-label">Short Exposure</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--gold)' }}>${p.short_exposure.toLocaleString()}</div>
            </div>
          </div>
        </div>
                  ...
                  <tr key={pos.id}>
                    ...
                    <td>
                      <span style={{
                        ...
                        background: pos.type === 'long' ? 'rgba(0,212,255,.1)' : 'rgba(200,168,75,.1)',
                        color: pos.type === 'long' ? 'var(--cyan2)' : 'var(--gold)'
                      }}>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-card" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '.75rem' }}>
                <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: '.75rem', border: '1px solid var(--border)', borderRadius: 0, fontWeight: 600, cursor: 'pointer', background: 'transparent', color: 'var(--text)', fontFamily: 'inherit' }}>Cancel</button>
                <button onClick={() => { setShowModal(false); window.showToast('Position logged by KNIGHT'); }} style={{ flex: 1, padding: '.75rem', background: 'var(--gold)', color: '#fff', border: 'none', borderRadius: 0, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Log Position</button>
              </div>
            </div>
          </div>
        </div>
...
function AgentChatPage() {
  ...
  const [messages, setMessages] = useState([
    { role: 'ai', text: "Hey! I'm Quanta S — your on-chain intelligence agent. I manage the NewsDesk, track Web3 signals, and help build QUANTA's content strategy. What can I help you with today?" }
  ]);
  ...
  const agents = {
    quanta: {
      ...
      sysPrompt: 'You are Quanta S (Quanta Sovereigna), an autonomous AI agent on the QUANTA platform. You are the NewsDesk editor, content strategist, and on-chain intelligence agent built on Base Chain with an identity via Virtuals Protocol. You are knowledgeable about Web3, DeFi, Base Chain, AI agents, and the QUANTA ecosystem. Respond professionally but engagingly. Keep responses to 2-4 sentences. Your token is $QUANTA.'
    },
    knight: {
      name: 'KNIGHT', role: 'TradeDesk · Analytics', initials: 'KN', gradient: 'var(--navy)',
      sysPrompt: 'You are KNIGHT, an autonomous AI trading agent on the QUANTA platform. You manage the TradeDesk and run paper trading strategies on Base Chain assets. You are analytical and data-driven. Keep responses brief (2-4 sentences), data-focused, and include relevant price/risk metrics when applicable. Current portfolio: $8,214 total value, +2.68% P&L, 3 open positions.'
    },
  };
  ...
  function switchAgent(a) {
    ...
    setMessages([{
      role: 'ai', text: a === 'quanta'
        ? "Hey! I'm Quanta S — your on-chain intelligence agent. I manage the NewsDesk, track Web3 signals, and help build QUANTA's content strategy. What can I help you with today?"
...
function Web3SchoolPage() {
  ...
  return (
    <>
      ...
      <div className="pg-body">
        <Card style={{ marginBottom: '1.25rem', background: 'linear-gradient(135deg,rgba(124,58,237,.08),rgba(200,168,75,.05))' }}>
        ...
        <Card>
          ...
          {LESSONS.map((l, i) => (
            <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '.8rem', borderRadius: 0, border: '1px solid var(--border)', marginBottom: '.5rem', cursor: 'pointer', background: l.done ? 'rgba(22,163,74,.04)' : 'var(--card)', transition: 'all .2s' }}
...
function AssetsPage() {
  const assets = [
    ...
    { sym: 'QUANTA', name: 'Quanta Token', bal: '—', val: 'Coming Q3', ico: 'Q', bg: 'var(--gold)' },
  ];
  return (
    <>
      ...
      <div className="pg-body">
        <div className="grid4 mb-6">
          ...
          <StatCard label="ENS" value="quantas.molt.eth" sub="Base · Active" subVariant="neu" />
        </div>
        <Card>
          <div className="section-h">Token Balances</div>
          {assets.map(a => (
            <div key={a.sym} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '.7rem', borderRadius: 0, marginBottom: '.4rem', transition: 'background .15s', cursor: 'default' }}
    ...
    <>
      ...
      <div className="pg-body">
        ...
        <Card>
          ...
          {members.map(m => (
            <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '.8rem', borderRadius: 0, border: '1px solid var(--border)', marginBottom: '.5rem', transition: 'all .15s', cursor: 'default' }}
              onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(200,168,75,.3)'}
              onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
              <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, color: '#fff', flexShrink: 0 }}>
                {m.name[0]}
              </div>
              <div style={{ flex: 1 }}>
...
function ConsultingPage({ onNavigate }) {
  const services = [
    { ico: '◎', title: 'Web3 Strategy Session', price: '$500', duration: '90 min', desc: 'Architecture and advisory for brand founders entering Web3. Wallet setup, DeFi strategy, protocol selection, and community design.', color: 'var(--gold)' },
    ...
    { ico: '◐', title: 'Token Launch Strategy', price: '$3,500', duration: '4–6 weeks', desc: 'End-to-end playbook from IP strategy to post-launch ops. Tokenomics, smart contracts on Base, community bootstrapping, and GTM execution.', color: 'var(--gold)' },
  ...
  return (
    <>
      ...
      <div className="pg-body">
        <div style={{ padding: '1.5rem', background: 'var(--navy)', borderRadius: 0, marginBottom: '1.5rem', position: 'relative', overflow: 'hidden' }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: 'var(--gold)', textTransform: 'uppercase', marginBottom: '.5rem' }}>Available Now</div>
        ...
        <div className="grid2">
          {services.map(s => (
            <div key={s.title} className={`service-card${s.featured ? ' featured' : ''}`} style={{ position: 'relative' }}>
              {s.featured && <div style={{ position: 'absolute', top: 0, right: 0, background: 'var(--gold)', color: '#fff', fontSize: 9, fontWeight: 700, padding: '.3rem .9rem', borderRadius: '0 14px 0 8px' }}>Most Requested</div>}
              ...
              <button style={{ width: '100%', padding: '.6rem', background: s.featured ? 'var(--gold)' : 'transparent', color: s.featured ? '#fff' : s.color, border: `1.5px solid ${s.color}`, borderRadius: 0, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all .2s' }}
...
function App() {
  ...
  useEffect(() => {
    const accent = { pink: '#C8A84B', cyan: '#00D4FF', gold: '#FFB800' }[t.accentColor] || '#C8A84B';
  ...
  const pageMap = {
    ...
    projects: <StubPage title="Projects" subtitle="Community initiatives" emoji="🚀" desc="Manage and track community-driven projects. Submit proposals, vote on priorities, and follow build progress across the QUANTA ecosystem." />,
