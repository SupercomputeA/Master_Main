...
const TRADEDESK_DEP_PATTERN = /^https:\/\/github\.com\/SupercomputeA\/supercompute-tradedesk\.git#(?:develop|[a-f0-9]{40})$/

assert.match(
  pkg.dependencies?.["@supercompute/tradedesk"] ?? "",
  TRADEDESK_DEP_PATTERN,
  "TradeDesk must be sourced from the tradedesk repository via HTTPS, pinned to develop or a full SHA (so CI can resolve the private repo with GITHUB_TOKEN)",
)
